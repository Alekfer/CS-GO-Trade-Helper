    ...
    makeAPICall('https://api.steampowered.com/IEconService/GetTradeOffers/v1', {
        ...
        data.response.trade_offers_received.forEach(function(offer) {
            ...
            if (offer.items_to_receive) {
                var theirItemsInTrade = offer.items_to_receive.map(function(item) {
                    return item.assetid
                })
            ...
            if (offer.items_to_give) {
                var myItemsInTrade = offer.items_to_give.map(function(item) {
                    return item.assetid
                })
...
function getPrices(callback) {
    ...
    var urls = {
        ...
        steamapiscom: 'http://api.steamapis.com/market/items/730?format=compact&api_key=',
        steamapiio: 'https://api.steamapi.io/market/prices/730?key='
    };

    /* get settings, 'prices' is the pricing source, 'steamlytics' is the Steamlytics api key */
    chrome.storage.sync.get(['prices', 'steamlytics', 'steamapiscom', 'steamapiio'], function(settings) {
        ...
        if (settings.hasOwnProperty('steamapiscom') && source == 'steamapiscom') {
            url += settings.steamapiscom
        }

        if (settings.hasOwnProperty('steamapiio') && source == 'steamapiio') {
            url += settings.steamapiio
        }

        $.ajax({
            url: url,
            success: function(response) {
                console.log(source, url, response)
                var tempPrices = {}
                if ((source === 'backpack' || source == 'bitskins') && !response.success) {
                    return setTimeout(getPrices.bind(null, callback), 1000)
                }

                /* steamlytics only allows one request per 10 minutes */
                if (source === 'steamlytics') {
                    if (!response.success) return setTimeout(getPrices.bind(null, callback), 1000 * 60 * 10)
                    else tempPrices = response.items
                }

                if (source === 'steamapiio' || source === 'steamapiscom') {
                    if (response) tempPrices = response
                }

                callback(prices = tempPrices)
            },
            error: function() {
