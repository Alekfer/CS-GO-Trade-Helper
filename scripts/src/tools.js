...
if (path.join('/') !== '/' && ['tradeoffer'].indexOf(path[1]) == -1 || ['chat'].indexOf(path[2]) > -1 && ['tradingcards'].indexOf(path[1]) == -1 && path.join('/').indexOf('/json/') == -1) {
    ...
    $('.profile_small_header_texture').css({
        'background-image': 'inherit',
        'background-color': 'rgba(26,41,58,0.75)',
...
function isOfferGlitched(offerID, callback) {
    makeAPICall('https://api.steampowered.com/IEconService/GetTradeOffer/v1/', {
        ...
        var descriptions = response.descriptions.map(function(item) {
            return item.classid
        })
    ...
    injectScript({'%%steamID%%': steamID, '%%steamID%%': steamID}, function() {
        ...
        $J('.itemHolder:not(.trade_slot)').each(function() {
            ...
            if (item && item.actions) {
                if (item.actions.some(function(item) {
                        return item.name === 'View on Exchange...' || item.name === 'View on Metjm...'
                    })) return;
                $J(this)[0].rgItem.actions.push({
                    link: 'https://csgo.exchange/item/' + item.id,
                    name: 'View on Exchange...'
                });
